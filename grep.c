...
int main(int argc, char* argv[]) {
  ...
  for (size_t i = 2; i<argc; i++){
    process_dir(argv[i], search_for, search_file);}                             // for loop to process multiple files
    }
    printf("\nquitting.......\n");
 return 0;
}
...
int advance(char *lp, char *ep) {  char *curlp;  int i;                         // Advance func check for the patteren in buffer
  for (;;) {                                                                    // cases for the line numbers
    switch (*ep++) {
      ...
      case CCHR|STAR:  curlp = lp;  while (*lp++ == *ep) { }  ++ep; do {  lp--;  if (advance(lp, ep)) { return(1); } } while (lp > curlp);
      return(0);
      case CCL|STAR:
...
int backref(int i, char *lp) {  char *bp;  bp = braslist[i];                    // fun for back refrence used in advance
...
void error(char *s) {  int c;  wrapp = 0;  listf = 0;  listn = 0;  putchar('?');// Error func used to throw an error
  puts_(s); count = 0;  lseek(0, (long)0, 2);  pflag = 0;
  if (globp) { lastc = '\n'; }  globp = 0;  peekc = lastc;
  if(lastc) { while ((c = getchr()) != '\n' && c != EOF) { } }
  if (io > 0) { close(io);  io = -1; }
}

int execute(unsigned int *addr) {  char *p1, *p2 = expbuf;  int c;              // execute func to find the pattren in the string line
...
int getchr(void) {  char c;                                                     // getch_ func is used in getchr func to read a char from a shared buf throw ungetch_ func
  ...
  if ((c =getch_()) < 0) { return(lastc = EOF); }  lastc = c & 0177; return lastc;// if loop to see if its the end of the file
}

int getfile(void) {  int c;  char *lp = linebuf, *fp = nextip;                  // getfile func to read char from the file.txt and save that text in genbnuf[]
  do {
    if (--ninbuf < 0) {
...
char* getline_blk(unsigned int tl) {  char *bp, *lp;  int nl;  lp = linebuf;  bp = getblock(tl, READ);
  nl = nleft;  tl &= ~((BLKSIZE/2)-1);                                          // func to get a newline
  while ((*lp++ = *bp++)) { if (--nl == 0) {  bp = getblock(tl+=(BLKSIZE/2), READ);  nl = nleft;  } }
  return(linebuf);
}

int getch_(void) { char c = (bufp > 0) ? buf[--bufp] : getchar();               // getch_ func to read character from a shared buf with ungetch_ func
...
void init(void) {  int *markp;  close(tfile);  tline = 2;                       // init func to initilize
  for (markp = names; markp < &names[26]; )  {  *markp++ = 0;  }
  subnewa = 0;  anymarks = 0;
  iblock = -1;  oblock = -1;  ichanged = 0;
  close(creat(tfname, 0600));  tfile = open(tfname, 2);
  dot = dol = zero;  memset(inputbuf, 0, sizeof(inputbuf));
}
...
void print(void) {  unsigned int *a1 = addr1;  nonzero();                       // func to print the characters
  do {  if (listn) {  count = a1 - zero; }  puts_(getline_blk(*a1++));  } while (a1 <= addr2);
...
void printcommand(void) {                                                       // printcommand func to print if the regexp pattren is found and then a EOF case to close the func
  int c;  char lastsep;
   for (;;) {  unsigned int* a1;
     ...
     for (addr1 = 0;;) {  lastsep = c;  a1 = 0;  c = getchr();
     ...
     switch(c) {                                                                // switch statements for p case aka print and EOF case aka end of file
       case 'p':  case 'P': match = 1; printf("\t"); print(); newline();
...
void quit(int n) { if (vflag && fchange && dol!=zero) {  fchange = 0;  error(Q);  }  unlink(tfname); exit(0); } // func to quit the procedure
...
void setnoaddr(void) { if (given) { error(Q); } }                               // set no address func to throw error if no address is found
